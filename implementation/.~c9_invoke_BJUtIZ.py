...
def register():
    ...
    if request.method == "POST":
        ...
        #    return apology("Please insert a valid email address", 400)
...
@app.route("/cancelclaim", methods=["GET", "POST"])
def cancelclaim(value, buyerid):
    # This works, but im not sure how to implement it
    # I want this to be the result of a user clicking a link to a page, and then clicking a confirmation button
    if request.method == "GET":
        item = db.execute("SELECT * FROM posted WHERE id= :id", id=value)
        return render_template("cancelrequest.html", item=item)
    if request.method == "POST":
        buyer = db.execute("SELECT * FROM users WHERE id= :id", id=buyerid)
        currency = buyer[0]["currency"]
        db.execute("UPDATE users SET currency= :currency WHERE id= :id", id=buyerid, currency=currency+1)
        db.execute("UPDATE posted SET avail= :avail, buyerid=NULL WHERE id= :id", avail=1, id=value)
...
def emailclaim(value):
    ...
    buyermsg = ("Hi {0}! This email is confirming that you requested to claim the item \'{1}\'. Below is the owner\'s information. \n Please contact them to arrange a drop off time and location for the item. Thank you!\n\n".format(buyer[0]["name"], item[0]["name"]) +
                "Name -- {0}\nEmail -- {1}\nPhone number -- {2}\nDorm/House -- {3}\nYear -- {4}\nItem requested -- {5}.\n\nIf you did not intend to request this item, click the link below\nhttp://ide50-juanmolina.cs50.io:8080/cancelclaim".format(seller[0]["name"], seller[0]["email"], seller[0]["pnumber"], seller[0]["dorm"],  seller[0]["year"], item[0]["name"]))
...
def browse():
    ...
    if request.method == "GET":
        ...
        # do i need the rows thing here???? i dont think so i might delete
        rows = db.execute("SELECT * FROM posted")
